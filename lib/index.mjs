...
export default class {
  static start({
    sshuser = process.env.SSHUSER || 'wettyuser',
    sshhost = process.env.SSHHOST || 'localhost',
    sshauth = process.env.SSHAUTH || 'wetty123456user',
    sshport = process.env.SSHPOST || 22,
    port = process.env.PORT || 31297,
    sslkey = process.env.SSHKEY || '/etc/ssl/certs/key.pem',
    sslcert = process.env.SSHCERT || '/etc/ssl/certs/cert.pem',
  }) {
    wetty
      .on('exit', ({ code, msg }) => {
